...
// changed the MFA to use crypto, as it is more secure then the current MFA setup
        if (user.mfa_enabled) {
            ...
            // Skip email sending in development mode for frontend testing
            await sendEmail(user, token);
            
            // For development: return success immediately without requiring MFA
            if (process.env.NODE_ENV === 'development') {
                ...
                return res.status(200).json({ 
                    user, 
                    token,
                    message: "Development mode: MFA bypassed for frontend testing"
                });
            }
...
async function sendEmail(user, token) {
    // Bypass email sending in development mode for frontend testing
    if (process.env.NODE_ENV === 'development' || !process.env.SENDGRID_KEY) {
        console.log(`=== DEVELOPMENT MODE: Email sending bypassed ===`);
        console.log(`Recipient: ${user.email}`);
        console.log(`MFA Token: ${token}`);
        console.log(`=== In production, user would receive this token via email ===`);
        return;
    }
    ...
    try {
        ...
        // Log token for development debugging even if email fails
        console.log(`Development debug - MFA Token: ${token}`);
    }
}
